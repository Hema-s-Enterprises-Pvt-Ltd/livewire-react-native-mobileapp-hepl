...
const CancelModal = ({visible, onClose,details}) => {
  const formattedDateTime = `${moment(details?.demo.scheduleDate).format("MMMM DD")} (${moment(details?.demo.scheduleTime, 'HH:mm').format('h:mm A')})`;
     ...
     propagateSwipe={true}
      ...
      <View style={styles.centeredView}>
        ...
        <View
          ...
          <View
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center',flexWrap:'wrap' }}>
              ...
              {details?.demo.serviceType?.name} • 
              </Text>
              <Text style={{ color: '#737973', fontSize: 12 }}>
                {' (' + moment(details?.demo.scheduleTime, ['HH:mm']).format('h:mm A') + ')'}
              </Text>
            </View>
          </View>
        </View>
        <View
          ...
          >
            {details?.demo.salesman?.profilePic ?  <Image
              source={{uri:details?.demo.salesman?.profilePic}}
              resizeMode={"stretch"}
              style={{
                borderRadius: 215,
                width: 35,
          ...
          <Text
            ...
            {details?.demo.salesman?.firstName}  {details?.demo.salesman?.lastName}
          </Text>
        </View>
        ...
        <View style={styles.detailContainer}>
          <View style={styles.detailChildContainer}>
            <DetailItem
              ...
              isheader={true}
            />
            {details?.demo.clientLocation && <DetailItem
              image={'Ionicon'}
              title={details?.demo.clientLocation?.location} 
              isheader={false}
            />}
            {details?.demo.services && <DetailItem
              image={'SimpleIcon'}
              title={details?.demo.services?.name} 
              isheader={false}
            />}
            {details?.demo.product && < DetailItem
              image={'FontAwesomefive'}
              title={details?.demo.product?.name} 
              isheader={false}
            />}
          </View>
